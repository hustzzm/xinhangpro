...
import {
  dynamicRoutesByProduct,
  dynamicButtonsByProduct,
  list,
  submit,
  detail,
  remove,
...
export default {
  ...
  effects: {
    *fetchMenuData({ payload }, { call, put }) {
      ...
      if (routes.length === 0 && (currentUser && currentUser.roleid)) {
        const response = yield call(dynamicRoutesByProduct,'nipt-plus');
      ...
      if (buttons.length === 0 && (currentUser && currentUser.roleid)) {
        const response = yield call(dynamicButtonsByProduct,'nipt-plus');
        //普通用户可能为空的情况
