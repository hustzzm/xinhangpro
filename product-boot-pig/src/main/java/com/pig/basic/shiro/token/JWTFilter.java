...
public class JWTFilter extends BasicHttpAuthenticationFilter {
    ...
    private boolean refreshToken(ServletRequest request, ServletResponse response) {
        ...
        if (RedisUtil.exists(SystemConstant.PREFIX_SHIRO_REFRESH_TOKEN + account)) {
            ...
            if (JWTUtil.getClaim(token, SystemConstant.CURRENT_TIME_MILLIS).equals(currentTimeMillisRedis)) {
                ...
                String refreshTokenExpireTime = PropertiesListenerConfig.getProperty("refreshTokenExpireTime");
                // 设置RefreshToken中的时间戳为当前最新时间戳，且刷新过期时间重新为30分钟过期(配置文件可配置refreshTokenExpireTime属性)
                RedisUtil.setObject(SystemConstant.PREFIX_SHIRO_REFRESH_TOKEN + account, currentTimeMillis, Integer.parseInt(refreshTokenExpireTime));
                // 刷新AccessToken，设置时间戳为当前最新时间戳
                token = JWTUtil.sign(account,userName, currentTimeMillis,roleId);
    ...
    private void response403(ServletResponse response, String msg) {
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = new CommonResult().failed( "鉴权失败:" + msg).toString();
            out.append(data);
        } catch (IOException e) {
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        httpServletResponse.setContentType("application/json; charset=utf-8");
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = new CommonResult().failed( "无权访问(Unauthorized):" + msg).toString();
            out.append(data);
        } catch (IOException e) {
            logger.error("直接返回Response信息出现IOException异常:" + e.getMessage());
            throw new RuntimeException("直接返回Response信息出现IOException异常:" + e.getMessage());
